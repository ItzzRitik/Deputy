const countries = [{
	...
	// Accepts countries `name` and returns its countryCode
	getCountryCodeByName = (name) => {
		if (name) {
			return countries.find((country) => new RegExp("\\b" + name + "\\b", 'i').test(country.name))?.iso2;
		}
	},
	...
	// Accepts countries `name` and returns its countryFlag
	getCountryFlagByName = (name) => {
		const countryCode = getCountryCodeByName(name);

		if (countryCode) {
			return getCountryFlagByCountryCode(countryCode);
		}
	},

	// Accepts countries `dialCode` and returns its countryFlag
	getCountryFlagByDialCode = (dialCode) => {
		const countryCode = getCountryCodeByDialCode(dialCode);
